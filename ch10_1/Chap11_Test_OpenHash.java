...
class SimpleObject2 {
	...
	void scanSimpleObject2 (String guide) {
...
class OpenHash<V> {
	...
	// --- 해시값을 구함 ---//
	public int hashValue(SimpleObject2 o) {
		int no = Integer.parseInt(o.sno);
		return no % size;
	}
	...
	// --- 키값 key를 갖는 버킷 검색 ---//
	private Bucket<V> searchNode(SimpleObject2 o, Comparator<? super SimpleObject2> c) {
		int hash = hashValue(o); // 검색할 데이터의 해시값
	...
	public V search(SimpleObject2 o, Comparator<? super SimpleObject2> c) {
		Bucket<V> p = searchNode(o, c);
		if (p != null)
			return p.getValue();
		else
			return null;
	...
	public int add(SimpleObject2 o, Comparator<? super SimpleObject2> c) {
		if (search(o, c) != null)
			return 1; // 키값이 이미 등록되어 있음

		int hash = hashValue(o); // 추가할 데이터의 해시값
		Bucket<V> p = table[hash]; // 주목 버킷
		for (int i = 0; i < size; i++) {
			if (p.stat == Status.EMPTY || p.stat == Status.DELETED) {
				p.set((V)o, Status.OCCUPIED);
				return 0;
			}
			hash = rehashValue(hash); // 재해시
			p = table[hash];
	...
	// --- 키값이 key인 요소를 삭제 ---//
	public int remove(SimpleObject2 o, Comparator<? super SimpleObject2> c) {
		Bucket<V> p = searchNode(o, c); // 주목 버킷
		if (p == null)
